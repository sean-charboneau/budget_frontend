...
(function() {
  ...
  ko.validation.register('gt', function(newValue, options) {
    return !isNaN(parseFloat(newValue)) && newValue <= ko.unwrap(options.val);
  }, "Value was higher than expected");
  ...
  ko.extenders.deferValidation = function(target, shouldDefer) {
    target.canValidate = ko.observable(false);
    if(!shouldDefer) {
        target.canValidate(true);
        return target;
    }
